...
const PlayVideo = () => {
    ...
    const fetchOtherData = async()=>{
        ...
        await fetch(channelDetail_url).then(response=>response.json()).then(data=>setChannelData(data.items[0]));
    ...
    <div className='play-video'>
        ...
        <div className="publisher">
            ...
            <div>
                ...
                <span>{channelData?valueConverter(channelData.statistics.subscriberCount):"0"}</span>
            </div>
            <button>Subscribe</button>
        </div>
        <div className="vid-description">
            <p>{apiData?apiData.snippet.description.slice(0,250):"Empty Description"}</p>
            <hr />
            <h4>{apiData?valueConverter(apiData.statistics.commentCount):"N/A"} Comments</h4>
